...
from database.models.user_token.crud import create_user_token, get_user_tokens, delete_user_token, update_user_token
...
tags_metadata = [
    {
        "name": "Admin",
        "description": "Функционал для администратора сервиса"
    },
    {
        "name": "User",
        "description": "Функционал для пользователя сервиса"
    },
]
...
@app.post("/user_tokens", tags=["Admin"], status_code=status.HTTP_201_CREATED,
...
@app.get("/user_tokens", tags=["Admin"], status_code=status.HTTP_200_OK,
         summary="Получение списка пользовательских токенов")
async def read_user_token(
        user: AdminAddDTO = Depends(get_admin),
        db: Session = Depends(get_db)
):
...
@app.put("/user_tokens/{user_token_id}", tags=["Admin"], status_code=status.HTTP_200_OK,
...
@app.delete("/user_tokens/{user_token_id}", tags=["Admin"], status_code=status.HTTP_200_OK,
...
@app.post("/model", tags=["Admin"], status_code=status.HTTP_201_CREATED,
...
@app.get("/model", tags=["Admin"], status_code=status.HTTP_200_OK,
...
@app.delete("/model", tags=["Admin"], status_code=status.HTTP_200_OK,
