...
async def user_token(token: str = Header(), db: Session = Depends(get_db)):
...
async def create_embeddings(data: EmbeddingRequestDTO, db: Session = Depends(get_db)):
...
async def chat(data: ChatCompletionRequestDTO, db: Session = Depends(get_db)):
    user = check_user(data.api_key, db)
